...
ggplot(all_2020, aes(newdate, pm25), jitter.x=TRUE, jitter.y=TRUE, auto.key=list(space="right")) + labs(x="Date", y="PM 2.5", title="test") +
geom_point(alpha=.4, size=.8, aes(colour=factor(pm25))) + 
...
boxplot(pm25~timegroup,data=all_four_timegroup, main="PM 2.5 Before and After Lockdown in 2020 and 2019", xlab="time groupings ranging 80 days each", ylab="PM 2.5") #works, but need to reoirent x axis

#failed attempt
#cities_means_pm25.csv = "/home/CAMPUS/dapa2019/R/China_air_pollution_COVID/cities_means_pm25.csv"
#cities_means_pm25 <- read.csv(cities_means_pm25.csv)
#boxplot(mean_pm25before2020~city,data=cities_means_pm25, main="Pollution Before and After 2020 Lockdowns", xlab="city", ylab="mean pm2.5")
