...
#presenting notes:
#can include concerns abt stat method, this is what I think i see"
#use vidgrid!
#50% of time is background and motivation.
...
75, 1, 1, 75, 0, 1, 75, 0, 0, 75, 2, 6,

75, 47, 103, 75, 63, 36, 75, 78, 6,

75, 16, 143, 75, 143, 145, 75, 28, 9,

75, 4, 15, 75, 0, 2, 76, 3, 1,

76, 28, 108, 76, 51, 117, 76, 17, 41,

76, 19, 23, 76, 40, 44, 76, 1, 4,

76, 2, 0, 77, 0, 0, 77, 0, 0,

77, 2, 26, 77, 2, 4, 77, 4, 10), 3)

 
#control is 2019, impact is 2020
 

baci.df = data.frame(Event=c(rep("Before_Lockdown", 12), rep("After_Lockdown", 13)), Year=abundance[1,], C_2019=abundance[2,], I_2020=abundance[3,]) #can take year out, maybe change to cityname

baci.df

str(baci.df)

t.test(baci.df$C_2019, baci.df$I_2020)

 

 

baci.df$delta = baci.df$C_2019 - baci.df$I_2020

 

t.test(baci.df$delta) #see if results are analagous

 

t.test(baci.df$delta[1:12], baci.df$delta[13:25]) #picks subset of data based on row names. is the delta different btween I and C?

boxplot(delta ~ Event, data=baci.df)

 

#  Better format for ANOVA
#USE THIS!!!

#baci.df2 = data.frame(Time=rep(1:25,2), Site=c(rep("C_2019", 25), rep("I_2020", 25)),

#Lockdown=rep(c(rep("Before", 12), rep("After", 13)),2),

#Year=rep(abundance[1,],2),

#PM25=c(abundance[2,], abundance[3,])); baci.df2

#baci.df2 #abundance is the actual data numbers. can create data set differently but needs to be same format. 
...
baci[Event=="before_lockdown",]
mean(Abundance[Event=="before_lockdown"]) #prints a number

baci[Event=="after_lockdown",]
mean(Abundance[Event=="after_lockdown"]) #prints NA???

SS_before = sum((mean(Abundance[Event=="before_lockdown"]) - Abundance[Event=="before_lockdown"])^2); SS_before
SS_after = sum((mean(Abundance[Event=="after_lockdown"]) - Abundance[Event=="after_lockdown"])^2); SS_after #prints NA

(SS_before + SS_after)/29 #should I use 29?

 

#2574*23

(length(Abundance[Event=="before_lockdown"])+ length(Abundance[Event=="after_lockdown"])-2)

str(baci)

 

baci.aov <- aov(Abundance ~ Event + Site + Event:Site, data=baci); summary(baci.aov) 
#what is Site?
#error: variable lengths differ (found for 'Event')

baci.aov2 <- aov(Abundance ~ Event + Error(factor(Time)) + Site + Event:Site, data=baci); summary(baci.aov2)
#error: variable lengths differ (found for 'Event')
...
baci.aov <- aov(delta ~ Event, data=baci.df); summary(baci.aov)
```
